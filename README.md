...
이 프로젝트는 **xAI Grok API**의 실시간 X(구 트위터) 검색 기능(`x_search`)을 활용하여, 최근 24시간 동안 X에서 가장 인기 있는(좋아요, 조회수, RT가 높은) 주식 투자 관련 게시글을 수집 및 요약한 뒤 **Slack** 채널로 전송해 주는 자동화 스크립트입니다.

---

## 🛠 주요 기능
- **실시간 X 트렌드 검색**: xAI Grok API의 서버사이드 X 검색(`x_search` tool)을 통해 최신 주식 관련 트윗을 실시간 검색 및 집계합니다.
- **AI 요약 & 가독성 향상**: Grok 모델이 단순히 링크만 주는 것이 아니라, 핵심 분석 내용과 주요 수치(조회수, 좋아요 등)를 가독성 높은 Slack 마크다운 포맷(mrkdwn)으로 요약합니다.
- **슬랙(Slack) 연동**: Incoming Webhook을 통해 지정된 슬랙 채널로 리포트를 매일 자동 발송합니다.
...
# 슬랙 인커밍 웹훅 URL (https://api.slack.com/messaging/webhooks 에서 생성 가능)
...
GitHub 저장소에 올려두면 매일 깃허브 서버에서 자동으로 실행되고 슬랙으로 전달되는 매우 편리한 방식입니다.

1. `.github/workflows/grok_slack_notifier.yml` 파일을 아래와 같이 생성합니다:
   ```yaml
...
3. **Repository secrets**에 `XAI_API_KEY`와 `SLACK_WEBHOOK_URL`을 등록합니다.
